...
// get all workouts
const getProductsInSaved = async (req, res) => {
  const { userId } = req.params;

  try {
    const products = await Saved.find({ userId }).sort({ createdAt: -1 });
...
// create a new workout
const addProducToSaved = async (req, res) => {
  const {userId,product_id,name,price,description,brand,image} = req.body

  // add to the database
  try {
    const product = await Saved.create({userId,product_id,name,price,description,brand,image})
